...
TIER_MULTIPLIERS = {'budget': 0.75, 'standard': 1.0, 'comfort': 1.0, 'premium': 1.35, 'luxury': 2.1}
...
def calculate_budget(selected_slugs, preferences=None, trip_details=None):
    ...
    stay_sublabels = {'hostel': f'{total_nights} Nights (Backpacker Hostels & Dorms)', 'budget_hotel': f'{total_nights} Nights (Budget Hotels & Guesthouses)', '3_star': f'{total_nights} Nights (3-Star & Boutique Coastal Stays)', '4_star': f'{total_nights} Nights (4-Star Resorts & Heritage Stays)', '5_star': f'{total_nights} Nights (5-Star Luxury Villas & Palaces)'}
    stay_sublabel = stay_sublabels.get(stay_type, f'{total_nights} Nights (3-Star Stays)')
    suggested_stays_list = [{'dest_name': d['name'], 'hotel_name': d['suggested_stay']['name'], 'area': d['suggested_stay']['area'], 'rating': d['suggested_stay']['rating']} for d in dest_objects if d.get('suggested_stay')]
    categories = [{'id': 'stay', 'name': 'Accommodation', 'icon': '🏨', 'percentage': 39, 'amount': cat_stay_amt, 'per_person': int(round(cat_stay_amt / travellers)), 'sublabel': stay_sublabel, 'suggested_stays': suggested_stays_list}, {'id': 'transit', 'name': 'Transportation', 'icon': '🚗', 'percentage': 19, 'amount': cat_transit_amt, 'per_person': int(round(cat_transit_amt / travellers)), 'sublabel': 'Cabs, Inter-city Rail / Expressways & Fuel'}, {'id': 'food', 'name': 'Food & Dining', 'icon': '🍲', 'percentage': 16, 'amount': cat_food_amt, 'per_person': int(round(cat_food_amt / travellers)), 'sublabel': 'Breakfasts, Highway Dhabas & Local Dining'}, {'id': 'activity', 'name': 'Activities & Sightseeing', 'icon': '🎟️', 'percentage': 15, 'amount': cat_activity_amt, 'per_person': int(round(cat_activity_amt / travellers)), 'sublabel': 'Entry Passes, Boat Rides & Adventure Sports'}, {'id': 'misc', 'name': 'Miscellaneous & Buffer', 'icon': '💳', 'percentage': 11, 'amount': cat_misc_amt, 'per_person': int(round(cat_misc_amt / travellers)), 'sublabel': 'Tolls, Parking, Tips & Emergency Contingency'}]
    day_spend_list = []
    ...
    for day_i in range(1, total_days + 1):
        ...
        day_spend_list.append({'day_num': day_i, 'location': day_loc, 'cost': day_cost, 'snippet': snippet})
    ...
    tier_definitions = [{'id': 'budget', 'title': 'Budget-Friendly', 'desc': 'Focus on essentials, hostels/guesthouses, and shared transit.', 'multiplier': TIER_MULTIPLIERS['budget'] * STAY_MULTIPLIERS['hostel']}, {'id': 'standard', 'title': 'Standard', 'desc': 'Balanced comfort, boutique 3-star stays, private rental cabs, and top experiences.', 'multiplier': TIER_MULTIPLIERS['standard'] * STAY_MULTIPLIERS['3_star']}, {'id': 'premium', 'title': 'Premium', 'desc': '4-star stays, dedicated chauffeur cab, fine dining, and guided activities.', 'multiplier': TIER_MULTIPLIERS['premium'] * STAY_MULTIPLIERS['4_star']}, {'id': 'luxury', 'title': 'Luxury', 'desc': '5-star luxury heritage villas, private boat charters & VIP trails.', 'multiplier': TIER_MULTIPLIERS['luxury'] * STAY_MULTIPLIERS['5_star']}]
    tier_comparisons = []
    for t in tier_definitions:
        ...
        tier_comparisons.append({'id': t['id'], 'title': t['title'], 'total_display': f'₹{t_total:,}', 'per_person_display': f'₹{t_per_person:,}', 'desc': t['desc'], 'is_active': t['id'] == budget_tier})
    dest_names = ' + '.join([d['name'] for d in dest_objects])
    date_context_str = f"{start_date.strftime('%d')}–{end_date.strftime('%d %B %Y')} ({total_days} Days · {total_nights} Nights)" if start_date else f'{total_days} Days · {total_nights} Nights'
    plan_tag_str = f'{budget_tier.capitalize()} Plan'
    return {'total_cost': total_cost, 'total_cost_display': f'₹{total_cost:,}', 'per_person': per_person, 'per_person_display': f'₹{per_person:,}', 'travellers': travellers, 'total_days': total_days, 'total_nights': total_nights, 'daily_avg_total': daily_avg_total, 'daily_avg_total_display': f'₹{daily_avg_total:,}', 'daily_avg_person': daily_avg_person, 'daily_avg_person_display': f'₹{daily_avg_person:,}', 'lower_estimate_display': f'₹{lower_est:,}', 'expected_estimate_display': f'₹{expected_est:,}', 'upper_estimate_display': f'₹{upper_est:,}', 'categories': categories, 'daily_spend': day_spend_list, 'tier_comparisons': tier_comparisons, 'destinations_display': dest_names, 'date_context_str': date_context_str, 'plan_tag_str': plan_tag_str, 'budget_tier': budget_tier}
