"""
Step 2 routes for DesiSafar: Travel style, budget, stay, and food preferences.
"""
...
def preferences_view():
    """Handle travel style preference selection and saving."""
    ...
    start_d, end_d, computed_days, _ = parse_dates_and_duration(
    ...
    avg_base_rate = (
        sum(d.get("base_cost_per_day", 3000) for d in selected_dest_objects) / len(selected_dest_objects)
    ...
    for tier_code, tier_mult in tier_multipliers.items():
        ...
        tier_estimates[tier_code] = {
            "total": t_cost,
            "total_display": f"{t_cost:,}",
            "lower": t_lower,
            "lower_display": f"{t_lower:,}",
            "upper": t_upper,
            "upper_display": f"{t_upper:,}",
            "per_person": t_person,
            "per_person_display": f"{t_person:,}",
        }
