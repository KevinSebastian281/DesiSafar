"""
Live HTTP End-to-End Test for DesiSafar Flask Server.
Simulates a complete real user session in browser using requests.Session.
"""
...
def run_e2e_flow():
    ...
    res = session.post(f"{BASE_URL}/destinations/update_trip", data={
        ...
        "return_date": "",  # left empty to test auto-calculation
        "travellers": "3"
    ...
    res = session.post(f"{BASE_URL}/preferences", data={
        "vibe": ["romantic", "foodie", "explorer"],
    ...
    for path in ["/", "/destinations", "/preferences", "/itinerary", "/budget"]:
        ...
        inline_events = re.findall(r'\son[a-z]+\s*=', page_res.text, re.IGNORECASE)
